...
`main.aux` (xr). `\newcommand{\blind}{0}` in `main.tex` produces the
unblinded version (Acknowledgments + AI-use disclosure). At template
adoption, move the section files into the official ASA/T&F JASA class
and switch the bibliography style to its agsm-style .bst; `apalike` is
the stand-in author-year style in this build.
